...
class DashboardController extends Controller
{
    ...
    public function index(SHUChart $lineChart, TransaksiChart $pieChart, AnggotaChart $lineChartAnggota, JenisAnggotaChart $pieChartJenisAnggota, Request $request)
    {
        ...
        if (Auth::user()->id_role == 1) {
            ...
            return view('pages.dashboard.index', compact('jumlahAnggota', 'jumlahPegawai', 'jumlahSimpanan', 'jumlahPinjaman', 'anggotaChart', 'jenisAnggotaChart', 'totalSimpananPokok', 'totalSimpananWajib', 'totalSimpananSukarela', 'pendapatan', 'shuChart', 'transaksiChart', 'pertumbuhanAnggota', 'anggotaTahun', 'shuTahun', 'transaksiTahun', 'selectedYear'));
        } else {
            $setor = DetailSimpanan::where('jenis_transaksi', '=', 'Setor')
                ->get();
            $tarik = DetailSimpanan::where('jenis_transaksi', '=', 'Tarik')
